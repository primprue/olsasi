...
export default function FilaDos() {
	...
	async function agregar() {
		...
		if (dcalculo1.faltadato === true) {
			...
			if (state.renglonanexo.length !== 0) {
				...
				setDatosRenglon([...datosrenglon, ...state.renglonanexo, datospresup[0]]);
			} else {
				setDatosRenglon([...datosrenglon, datospresup[0]]);
			}
		}
	...
	let textdata = [];
	if (state.stkrubro !== undefined) {
		if (state.stkrubro.length > 0) {
			textdata = [{
				id: "StkRubroAbr",
				label: "Rubro",
				value: state.stkrubro[0].StkRubroAbr,
	...
	return (
		<>
			<Grid >
				...
				{console.log('state.stkrubro  ', state.stkrubro)}
				{console.log('state.stkrubro.length  ', state.stkrubro.length)}
				<Grid container size={{ xs: 1 }}>

					{/* {rubrosn === "S" &&

						state.stkrubro.length > 0 && ( */}
					{rubrosn === "S" && state.stkrubro.length > 0 && (
