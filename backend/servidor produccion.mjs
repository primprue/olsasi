...
// 1. Definimos la ruta de DIST (donde está tu React build) esto para produccion
...
const RUTA_PUBLICOS = process.env.RUTA_INTERNA_DOC ? path.join(process.env.RUTA_INTERNA_DOC, 'publicos', 'public') : null;
...
app.get('*', (req, res) => {
    ...
    res.sendFile(archivoIndex, (err) => {
        if (err) {
            ...
            res.status(404).send("Error: No se encuentra la carpeta dist o el index.html de Sistema");
        }
    });
});
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`🚀 Sistema activo en puerto ${PORT} (0.0.0.0)`);
});