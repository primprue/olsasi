...
router.post("/", function (req, res, next) {
    ...
    var dolaressn = req.body.dolaressn
    ...
    if (TotalPresup === 0) {
        if (descrip === '') {
            ...
            datospresup.map(reng => {
                ...
                var ImpUnit = { text: formato.format(reng.ImpUnitario).toString(), style: 'tableDatosD' }
                var ImpItem = { text: formato.format(reng.ImpItem).toString(), style: 'tableDatosD' }
        ...
        if (descrip === '') {
            ...
            var ImpItem = { text: formato.format(TotalPresup).toString(), style: 'textoDTot' }
  ...
  if (TipoPresup === 'BOLSON PARA TANQUE') {
        ...
        notatanque.push([{
            text: '	El bolsón cotizado es una lámina impermeable, no es, un contenedor de líquido. ' +
                'Por tal motivo, debe quedar apoyado sobre una superficie lisa que no tenga porosidad o rugosidad ya que ese tipo de superficie lo ' + 
                'perforaría. El agua en el fondo del tanque, una vez lleno, ejerce un peso mayor en la parte más profunda (piso y pared), y van ' +
                'disminuyendo los kilos de presión hacia la parte superior.', style: 'resaltado'
        }])
    }
   ...
   //  var top = 30;
    var fonts = {
        Roboto: {
            normal: '/home/sandra/SIOLSA/OlsaSG/node_modules/pdfmake/fonts/Roboto-Regular.ttf',
            bold: '/home/sandra/SIOLSA/OlsaSG/node_modules/pdfmake/fonts/Roboto-Medium.ttf',
