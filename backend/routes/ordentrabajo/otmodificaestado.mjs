...
router.post("/", async (req, rest) => {
    ...
    const OTEncabEstado = req.body.OTEncabEstado;

    try {
        const q = `Update BasesOrdenes.OTEncab SET OTEncabEstado = ?
                    where idOTEncab = ?`;
        await conexionpool.query(q, [OTEncabEstado, indice]);
        return rest.status(200).json({
            leyenda: 'OT Estado modificado correctamente',
        });
    } catch (err) {
