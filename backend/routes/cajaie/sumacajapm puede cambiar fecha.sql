...
FROM (
    SELECT 
        ...
        SUM(CASE WHEN c.CajaIEMT = 'M' AND DATE(c.CajaIEFecha) = curdate() - INTERVAL 1 DAY THEN c.CajaIEImporte ELSE 0 END) AS totalM,
        SUM(CASE WHEN c.CajaIEMT = 'T' AND DATE(c.CajaIEFecha) = curdate()  - INTERVAL 1 DAY THEN c.CajaIEImporte ELSE 0 END) AS totalT,
        SUM(CASE WHEN c.CajaIEMT = 'T' AND c.CajaIEImpIP <> 0 AND c.CajaIECodIP <> 'EFC' AND DATE(c.CajaIEFecha) = curdate()  - INTERVAL 1 DAY THEN c.CajaIEImpIP ELSE 0 END) AS totalInstr,
